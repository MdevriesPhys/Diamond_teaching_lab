...
def hahn_pulse_creation(
    ...
    """
    Signal half:
        laser init -> wait -> pi/2 -> tau -> pi -> tau -> pi/2 -> wait -> laser readout

    Reference half:
        same total timing but no MW
    """
    ...
    if signal_used >= half_us:
        raise ValueError(
            ...
            "Increase tref_ms or reduce tau/pi."
        )
    if ref_used >= half_us:
        raise ValueError(
    ...
    pb_inst_pbonly(CH_REF | CH_MW_I, CONTINUE, 0, _ns(pi_us))
    pb_inst_pbonly(CH_REF, CONTINUE, 0, _ns(tau_us))
    pb_inst_pbonly(CH_REF | CH_MW_I, CONTINUE, 0, _ns(pi2_us))
    ...
    # no MW in reference half, but keep same time envelope
    pb_inst_pbonly(0, CONTINUE, 0, _ns(2 * tau_us + 2 * pi2_us + pi_us + post_wait_us))
